...
export function isProgressCreationAcceptable(
  enqueueSnackbar: any,
  ...
  const doesOverlapProgress = progresses.filter((progressElement) => {
    if (progressElement.opsid === opsid) {
      ...
      const range2 =
        progressElement.fineavanzamento !== "0000-00-00 00:00:00"
          ? moment.range(
              moment(progressElement.inizioavanzamento, "YYYY-MM-DD HH:mm:ss"),
